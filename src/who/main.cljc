(ns who.main
  "Kotodama WASM entrypoint for the Who clean-room actor (L4) — Clojure port.
  ...
  facts (`who.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"who.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "who-compat" :tier tier :entities entities} 200])
